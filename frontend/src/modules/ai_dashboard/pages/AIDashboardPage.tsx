...
function RiskBar({ value, threshold = 60 }: { value: number; threshold?: number }) {
  ...
  const color = danger ? T.errorTxt : value >= 40 ? T.warnTxt : T.successTxt
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-semibold font-mono w-12 text-right" style={{ color }}>
        {pct(value)}
      </span>
    </div>
  )
...
const thStyle: React.CSSProperties = { color: T.textMuted, borderBottom: `1px solid ${T.borderSub}`, backgroundColor: T.input }
const tdCls = 'px-4 py-3 text-sm'
const dividerStyle: React.CSSProperties = { borderTop: `1px solid ${T.borderSub}` }
    ...
    <div className="flex mb-6" style={{ borderBottom: `1px solid ${T.border}` }}>
      {tabs.map((t) => (
        <button
          ...
          onMouseEnter={e => { if (active !== t.id) e.currentTarget.style.color = T.text }}
...
function ViewAllModal({ type, onClose }: { type: ModalType; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 pt-16 px-4 pb-4 overflow-y-auto backdrop-blur-sm">
      <div className="w-full max-w-5xl rounded-2xl shadow-2xl" style={{ backgroundColor: T.surface, border: `1px solid ${T.border}` }}>
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: `1px solid ${T.borderSub}` }}>
          <div className="flex items-center gap-2">
            <AlertTriangle className="h-5 w-5" style={{ color: T.errorTxt }} />
        ...
        <div className="p-6">
          {isLoading ? (
            <div className="py-12 text-center text-sm" style={{ color: T.textMuted }}>Loading all records…</div>
          ) : rows.length === 0 ? (
            <div className="py-12 text-center text-sm rounded-xl" style={{ color: T.textMuted, border: `1px dashed ${T.border}` }}>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {rows.map((r: any, i: number) => (
                  <tr key={r.employee_id} style={{ ...dividerStyle, backgroundColor: 'rgba(42,26,26,0.4)' }}>
                    <td className={tdCls} style={{ color: T.textMuted, fontFamily: 'monospace' }}>{i + 1}</td>
                    <td className={tdCls} style={{ color: T.errorTxt, fontWeight: 500 }}>{r.full_name}</td>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {rows.map((r: any, i: number) => (
                  <tr key={r.expense_line_id} style={{ ...dividerStyle, backgroundColor: 'rgba(42,26,26,0.4)' }}>
                    <td className={tdCls} style={{ color: T.textMuted, fontFamily: 'monospace' }}>{i + 1}</td>
                    <td className={tdCls} style={{ color: T.errorTxt, fontWeight: 500 }}>{r.employee_name}</td>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {rows.map((r: any, i: number) => (
                  <tr key={r.payslip_id} style={{ ...dividerStyle, backgroundColor: 'rgba(42,26,26,0.4)' }}>
                    <td className={tdCls} style={{ color: T.textMuted, fontFamily: 'monospace' }}>{i + 1}</td>
                    <td className={tdCls} style={{ color: T.errorTxt, fontWeight: 500 }}>{r.full_name}</td>
                    <td className={tdCls} style={{ color: T.textMuted, fontFamily: 'monospace', fontSize: '0.75rem' }}>{r.employee_number}</td>
...
function ModelsTab() {
  ...
  if (isLoading) return <div className="py-8 text-center text-sm" style={{ color: T.textMuted }}>Loading…</div>
  ...
  return (
    <div className="space-y-4">
      ...
      {MODEL_TYPES.map((mt) => {
        ...
        return (
          <div key={mt.value} className="rounded-xl p-5" style={{ border: `1px solid ${T.border}`, backgroundColor: T.surface }}>
            <div className="flex items-center gap-2 mb-1">
              ...
              {active
                ...
                : <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ backgroundColor: T.elevated, color: T.textMuted }}>not trained</span>}
            </div>
            {active && (
              <p className="text-xs font-mono mb-3" style={{ color: T.textMuted }}>
            ...
            {active?.metrics?.length > 0 && (() => {
              ...
              return (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {stats.map((s) => (
                    <div key={s.label} className="rounded-lg p-3 text-center" style={{ backgroundColor: T.input }}>
...
function InsightsTab() {
  ...
  const tableWrap: React.CSSProperties = { border: `1px solid ${T.border}`, borderRadius: '0.75rem', overflow: 'hidden' }
    ...
    <div className="space-y-10">
      ...
      <section>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <ShieldAlert className="h-5 w-5" style={{ color: T.errorTxt }} />
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => attrition.refetch()} className="flex items-center gap-1 text-xs" style={{ color: T.textMuted }}>
              <RefreshCw className="h-3 w-3" /> Refresh
            </button>
          </div>
        </div>
        ...
        {attrition.isLoading ? (
          <div className="py-6 text-center text-sm" style={{ color: T.textMuted }}>Scoring employees…</div>
        ) : attrition.isError ? (
          <div className="py-4 text-sm" style={{ color: T.errorTxt }}>Model not trained — go to Model Registry and train attrition_predictor.</div>
        ) : (
          <div style={tableWrap}>
            <table className="min-w-full text-sm">
              <thead>
                <tr>
                  {['#', 'Employee', 'Department', 'Attrition Risk'].map((h) => (
                    <th key={h} className={thCls} style={thStyle}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(attrition.data ?? []).map((emp: any, i: number) => (
                  <tr key={emp.employee_id} style={{ ...dividerStyle, backgroundColor: emp.risk_pct >= 60 ? 'rgba(42,26,26,0.4)' : undefined }}>
                    <td className={tdCls} style={{ color: T.textMuted, fontFamily: 'monospace', fontSize: '0.75rem' }}>{i + 1}</td>
                    <td className={tdCls}>
                      <p className="font-medium" style={{ color: emp.risk_pct >= 60 ? T.errorTxt : T.text }}>{emp.full_name}</p>
                      <p className="text-xs" style={{ color: T.textMuted }}>{emp.employee_number}</p>
                    </td>
      ...
      <section>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <AlertTriangle className="h-5 w-5" style={{ color: T.warnTxt }} />
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => expenses.refetch()} className="flex items-center gap-1 text-xs" style={{ color: T.textMuted }}>
              <RefreshCw className="h-3 w-3" /> Refresh
            </button>
          </div>
        </div>
        ...
        {expenses.isLoading ? (
          <div className="py-6 text-center text-sm" style={{ color: T.textMuted }}>Scoring expense lines…</div>
        ) : expenses.isError ? (
          <div className="py-4 text-sm" style={{ color: T.errorTxt }}>Model not trained — train expense_violation first.</div>
        ) : (
          <div style={tableWrap}>
            <table className="min-w-full text-sm">
              <thead>
              ...
              <tbody>
                {(expenses.data ?? []).map((line: any, i: number) => (
                  <tr key={line.expense_line_id} style={{ ...dividerStyle, backgroundColor: line.risk_pct >= 60 ? 'rgba(42,26,26,0.4)' : undefined }}>
                    <td className={tdCls} style={{ color: T.textMuted, fontFamily: 'monospace', fontSize: '0.75rem' }}>{i + 1}</td>
      ...
      <section>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <Brain className="h-5 w-5" style={{ color: '#8888df' }} />
            <h2 className="text-base font-semibold" style={{ color: T.text }}>Top 10 — Invoice Classifications</h2>
          </div>
          <div className="flex items-center gap-4">
            <button onClick={() => invoices.refetch()} className="flex items-center gap-1 text-xs" style={{ color: T.textMuted }}>
              <RefreshCw className="h-3 w-3" /> Refresh
            </button>
          </div>
        </div>
        ...
        {invoices.isLoading ? (
          <div className="py-6 text-center text-sm" style={{ color: T.textMuted }}>Classifying invoices…</div>
        ) : invoices.isError ? (
          <div className="py-4 text-sm" style={{ color: T.errorTxt }}>Model not trained — train invoice_classifier first.</div>
        ) : (
          <div style={tableWrap}>
            <table className="min-w-full text-sm">
              <thead>
                <tr>
                  {['#', 'Invoice', 'Vendor', 'Amount', 'Actual Category', 'Predicted', 'Confidence'].map((h) => (
                    <th key={h} className={thCls} style={thStyle}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(invoices.data ?? []).map((inv: any, i: number) => (
                  <tr key={inv.invoice_id} style={{ ...dividerStyle, backgroundColor: !inv.is_match ? 'rgba(42,32,8,0.4)' : undefined }}>
                    <td className={tdCls} style={{ color: T.textMuted, fontFamily: 'monospace', fontSize: '0.75rem' }}>{i + 1}</td>
      ...
      <section>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            <AlertTriangle className="h-5 w-5" style={{ color: '#bf80df' }} />
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => payroll.refetch()} className="flex items-center gap-1 text-xs" style={{ color: T.textMuted }}>
              <RefreshCw className="h-3 w-3" /> Refresh
            </button>
          </div>
        </div>
        ...
        {payroll.isLoading ? (
          <div className="py-6 text-center text-sm" style={{ color: T.textMuted }}>Scoring payslips…</div>
        ) : payroll.isError ? (
          <div className="py-4 text-sm" style={{ color: T.errorTxt }}>Model not trained — train payroll_anomaly first.</div>
        ) : (
          <div style={tableWrap}>
            <table className="min-w-full text-sm">
              <thead>
                <tr>
                  {['#', 'Employee', 'Gross Pay', 'Net Pay', 'Pay Period', 'Anomaly Risk'].map((h) => (
                    <th key={h} className={thCls} style={thStyle}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(payroll.data ?? []).map((ps: any, i: number) => (
                  <tr key={ps.payslip_id} style={{ ...dividerStyle, backgroundColor: ps.risk_pct >= 60 ? 'rgba(42,26,26,0.4)' : undefined }}>
                    <td className={tdCls} style={{ color: T.textMuted, fontFamily: 'monospace', fontSize: '0.75rem' }}>{i + 1}</td>
                    <td className={tdCls}>
                      <p className="font-medium" style={{ color: ps.risk_pct >= 60 ? T.errorTxt : T.text }}>{ps.full_name}</p>
                      <p className="text-xs" style={{ color: T.textMuted }}>{ps.employee_number}</p>
                    </td>
...
function LogTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        <div className="py-8 text-center text-sm" style={{ color: T.textMuted }}>Loading…</div>
      ) : logs.length === 0 ? (
        <div className="py-12 text-center text-sm rounded-xl" style={{ color: T.textMuted, border: `1px dashed ${T.border}` }}>
          No predictions logged yet.
        </div>
      ) : (
        <div style={{ border: `1px solid ${T.border}`, borderRadius: '0.75rem', overflow: 'hidden' }}>
          <table className="min-w-full text-sm">
            <thead>
              <tr>
              ...
              {logs.map((log, i) => {
                ...
                return (
                  <tr key={log.id} style={{ borderTop: i > 0 ? `1px solid ${T.borderSub}` : undefined }}>
                    <td className={tdCls} style={{ fontFamily: 'monospace', fontSize: '0.75rem', color: T.textMuted }}>{log.model_id.slice(0, 8)}…</td>
                    ...
                    <td className={tdCls} style={{ fontFamily: 'monospace', fontSize: '0.75rem', color: T.textMuted }}>{log.latency_ms}ms</td>
      ...
      {data?.pages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-2">
          ...
          <button
            ...
            style={{ ...btnStyle, opacity: page === data.pages ? 0.4 : 1 }}
          >
            Next
          </button>
        </div>
